...
void SvS_GPU(FixedIntCounter* queryLength, FixedIntCounter* idToIndexMap, char* outputPath, unsigned int** queries, InvertedIndex* index, FILE * fp){
	int i, j;
	int id = -1;
	...
	while((id = nextIndexFixedIntCounter(queryLength, id)) != -1) {
	  ...
	  int qlen = queryLength->counter[id];
	  ...
	  for(i = 0; i < qlen; i++) {
		qStartPointers[i] = getStartPointer(index->pointers, queries[qindex][sortedDfIndex[i]]);
	  ...
	  if(algorithm == SVS) {
		...
		set = intersectSvS(index->pool, qStartPointers, qlen, minimumDf);
		  ...
		  printf("Algorithm should be SvS now, Not possible!\n");
		  exit(0);
	  }
